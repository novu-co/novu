...
export function initNovuClassSdk(session: UserSession, shouldRetry: boolean = false): Novu {
  const options: SDKOptions = { security: { apiKey: session.apiKey }, serverURL: session.serverUrl };
  if (!shouldRetry) {
...
export function initNovuFunctionSdk(session: UserSession): NovuCore {
  return new NovuCore({ security: { apiKey: session.apiKey }, serverURL: session.serverUrl });
}
