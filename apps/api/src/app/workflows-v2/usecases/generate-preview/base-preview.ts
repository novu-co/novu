...
import { IPreviewHandler } from './generate-preview.usecase';

const LOG_CONTEXT = 'GeneratePreviewUsecase';

@Injectable()
export abstract class BasePreviewHandler implements IPreviewHandler {
  constructor(
  ...
  protected abstract sanitizeControlValues(
    controlValues: Record<string, unknown>,
    stepData: StepResponseDto
  ): Record<string, unknown>;

  protected abstract mergeVariablesExample(
  ...
  async preview(command: GeneratePreviewCommand): Promise<GeneratePreviewResponseDto> {
    try {
      const {
        ...
        variableSchema,
        workflow,
        variablesExample: commandVariablesExample,
      } = await this.initializePreviewContext(command);

      const sanitizedValidatedControls = this.sanitizeControlValues(initialControlValues, stepData);
      ...
      const mergedVariablesExample = this.mergeVariablesExample(previewTemplateData, commandVariablesExample, workflow);
  ...
  private async initializePreviewContext(command: GeneratePreviewCommand) {
    ...
    const variablesExample = command.generatePreviewRequestDto.previewPayload;

    return { stepData, controlValues, variableSchema, workflow, variablesExample };
  }
