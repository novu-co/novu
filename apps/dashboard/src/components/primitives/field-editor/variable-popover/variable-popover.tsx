...
export function VariablePopover({ variable, onClose, onUpdate }: VariablePopoverProps) {
  ...
  const handleRawLiquidChange = (value: string) => {
    // Remove {{ and }} and trim if they exist, but don't require them
    ...
    parts.slice(1).forEach((part) => {
      if (part.startsWith('default:')) {
        // Extract default value, handling quotes and escaped quotes
        const defaultMatch =
        ...
        if (defaultMatch) {
          // Handle CodeMirror's escaping by removing one level of escaping
          const newDefaultVal = defaultMatch[1].replace(/\\\\'/g, "\\'").replace(/\\'/g, "'");
          setDefaultVal(newDefaultVal);
        }
      }
    });
